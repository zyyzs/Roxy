...
public abstract class EntityLivingBase extends Entity
{
    ...
    protected void jump() {
        ...
        if (this instanceof EntityPlayerSP) {
            JumpEvent event = new JumpEvent(jumpMotion, this.movementYaw);
    ...
    public void onLivingUpdate()
    {
        ...
        if (Math.abs(this.motionX) < PacketFixer.fixMotionJump()) {
            this.motionX = 0.0D;
        }

        if (Math.abs(this.motionY) < PacketFixer.fixMotionJump()) {
            this.motionY = 0.0D;
        }

        if (Math.abs(this.motionZ) < PacketFixer.fixMotionJump()) {
            this.motionZ = 0.0D;
        }
