...
import lol.tgformat.module.impl.render.ToolTipsAnim;
import lol.tgformat.ui.utils.RoundedUtil;
import lol.tgformat.utils.render.BlurBuffer;
...
public class GuiIngame extends Gui
{
    ...
    private float anim = 0, animUp, animDown, animItem;
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        ModuleManager.getModule(NotificationsMod.class).render();
    ...
    protected void renderTooltip(final ScaledResolution sr, float partialTicks) {
        final boolean animModEnable = ToolTipsAnim.getInstance.isState();
        if (animModEnable) {
            if (this.mc.getRenderViewEntity() instanceof EntityPlayer entityplayer) {
                final float needUp = 3;
                GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                animUp = sr.getScaledHeight() - (19 + needUp);
                final float i = (float) sr.getScaledWidth() / 2;
                float f = zLevel;
                zLevel = -90.0F;
                anim = AnimationUtil.moveUDFaster(anim, entityplayer.inventory.currentItem * 20);
                RoundedUtil.drawRound(i - 90,
                        animUp,
                        90 << 1,
                        20, 6,
                        new Color(0, 0, 0, 150));
                RoundedUtil.drawRound(i - 90 + anim,
                        animUp,
                        20,
                        20, 6,
                        new Color(233, 233, 233, 100));
                zLevel = f;
                ...
                GlStateManager.pushMatrix();
                int l = (int) (sr.getScaledHeight() - 15 - 3 - needUp);
                for (int j = 0; j < 9; ++j) {
                    float k = (float) sr.getScaledWidth() / 2 - 90 + j * 20 + 2;
                    renderHotbarItem(j, (int) k, l, partialTicks, entityplayer);
                }
                GlStateManager.popMatrix();
            ...
            if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
                ...
                this.drawTexturedModalRect(i - 91 - 1 + Client.instance.getSlotSpoofComponent().getSpoofedSlot() * 20, sr.getScaledHeight() - 22 - 1, 0, 22, 24, 22);
                this.zLevel = f;
                GlStateManager.enableRescaleNormal();
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
