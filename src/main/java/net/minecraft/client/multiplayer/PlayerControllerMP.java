...
public class PlayerControllerMP
{
    ...
    public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn, boolean sendNoEvent) {
        ...
        if (!sendNoEvent) {
            ...
            PacketUtil.sendPacketNoEvent(new C08PacketPlayerBlockPlacement(playerIn.inventory.getCurrentItem()));
        }
        int i = itemStackIn.stackSize;
        ItemStack itemstack = itemStackIn.useItemRightClick(worldIn, playerIn);
        if (itemstack != itemStackIn || itemstack.stackSize != i) {
    ...
    {
        short short1 = playerIn.openContainer.getNextTransactionID(playerIn.inventory);
        this.netClientHandler.addToSendQueue(new C0EPacketClickWindow(windowId, slotId, mouseButtonClicked, mode, Minecraft.getMinecraft().thePlayer.inventory.getStackInSlot(slotId), short1));
    }
