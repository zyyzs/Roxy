...
public class ItemRenderer
{
    ...
    private void doBlockTransformations(final float qq) {
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            Animations animations = ModuleManager.getModule(Animations.class);

            if (this.itemToRender != null) {
                final boolean canblock = animations.fake.isEnabled();

                if (this.itemToRender.getItem() instanceof ItemMap) {
                    renderItemMap(abstractclientplayer, f3, f, swingProgress);

                } else if (abstractclientplayer.getItemInUseCount() > 0 || canblock || ModuleManager.getModule(KillAura.class).isBlocking()) {
                        ...
                        case BLOCK: {
                            if (animations.isState()) {
                                if (mc.thePlayer.isUsingItem()) {
                                    GL11.glTranslated(animations.blockingX.getValue(), animations.blockingY.getValue(), animations.blockingZ.getValue());
                                }
                            ...
                            if (animations.isState()) {
                                ...
                                if (animations.mode.is("Exhibition")) {
                                    final float f5 = MathHelper.sin((float) (MathHelper.sqrt_float(swingProgress) * 3.1));
                                ...
                                if (animations.mode.is("Jello")) {
                                    ...
                                    GlStateManager.rotate(40.0F, -0.1f, 2 + v * 5f, v * 5);
                                ...
                                if (animations.mode.is("Styles")) {
                                    ...
                                    GlStateManager.rotate(-80.0F, 1.0F, 0.0F, 0.0F);
                                    GlStateManager.rotate(60.0F, 0.0F, 1.0F, 0.0F);
                                    float var11 = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * 3.14159265358979323846F);
                                ...
                                if (animations.mode.is("Leaked")) {
                                    this.transformFirstPersonItem(f, 0.0F);
                                    double progress = Math.abs(1 - timer.hasTimeLeft(300) / 300);
                                    double offset = (-Math.pow(progress < 1 ? progress - 0.5 : 0.5, 2) + 0.25) * 100;
                                    GlStateManager.translate(0, 0, 0);
                                    GlStateManager.rotate((float) (offset + 40.0F), 0.0F, 1.0F, 0.0F);
                                    GlStateManager.rotate(-92.0F, 1.0F, 0.0F, 0.0F);
                                    GlStateManager.rotate(53.0F, 0.0F, 1.0F + v * 5f, v * 5);
                                }
                                if (animations.mode.is("Sigma")) {
                                    this.transformFirstPersonItem(f * 0.5f, 0);
                                    GlStateManager.rotate(-sb * 55 / 2.0F, -8.0F, -0.0F, 9.0F);
                                    GlStateManager.rotate(-sb * 45, 1.0F, sb / 2, -0.0F);
                                    this.doBlockTransformations();
                                    GL11.glTranslated(1.2, 0.3, 0.5);
                                    GL11.glTranslatef(-1, this.mc.thePlayer.isSneaking() ? -0.1F : -0.2F, 0.2F);
                                }
                                if (animations.mode.is("1.7")) {
                ...
                renderPlayerArm(abstractclientplayer, f, swingProgress);
            }
            GL11.glPopMatrix();
    ...
    public void updateEquippedItem()
    {
        ...
        EntityPlayer entityplayer = this.mc.thePlayer;
        ItemStack itemstack = Client.instance.getSlotSpoofComponent().getSpoofedStack();
        boolean flag = false;
        ...
        if (this.itemToRender != null && itemstack != null)
        {
            if (!this.itemToRender.getIsItemStackEqual(itemstack))
            {
                ...
                {
                    boolean flag1 = Reflector.callBoolean(this.itemToRender.getItem(), Reflector.ForgeItem_shouldCauseReequipAnimation, new Object[] {this.itemToRender, itemstack, Boolean.valueOf(this.equippedItemSlot != Client.instance.getSlotSpoofComponent().getSpoofedSlot())});

                    if (!flag1)
                    {
                        this.itemToRender = itemstack;
                        this.equippedItemSlot = Client.instance.getSlotSpoofComponent().getSpoofedSlot();
                        return;
                    }
                }
