...
public class DropdownObject implements Screen {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (bypass) {
            hoveringMainRect = RenderUtil.isHovering(x, y, width, height, mouseX, mouseY);
        }
        ...
        if (!openAnimation.isDone() || opened) {
            ...
            for (String option : options) {
                ...
                if (bypass) {
                    hovering = RenderUtil.isHovering(getX(), dropdownY + seperation, getWidth(), getHeight(), mouseX, mouseY);
                }
        ...
        lol.tgformat.ui.utils.RenderUtil.rotateStart(iconX, iconY, iconFont20.getStringWidth(FontUtil.DROPDOWN_ARROW), iconFont20.getHeight(), 180 * openAnimation.getOutput().floatValue());
        iconFont20.drawString(FontUtil.DROPDOWN_ARROW, getX() + getWidth() - 10, getY() + iconFont20.getMiddleOfBox(getHeight()) + 1, textColor);
        lol.tgformat.ui.utils.RenderUtil.rotateEnd();
