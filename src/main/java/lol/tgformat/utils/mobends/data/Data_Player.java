package lol.tgformat.utils.mobends.data;

import lol.tgformat.Client;
import lol.tgformat.module.ModuleManager;
import lol.tgformat.module.impl.render.MoBends;
...
import lol.tgformat.utils.mobends.client.renderer.SwordTrail;
...
public class Data_Player extends EntityData {
    ...
    public void syncModelInfo(ModelBendsPlayer argModel) {
        if (this.head == null) this.head = new ModelRendererBends(argModel);
        this.head.sync((ModelRendererBends) argModel.bipedHead);
        if (this.headwear == null) this.headwear = new ModelRendererBends(argModel);
        this.headwear.sync((ModelRendererBends) argModel.bipedHeadwear);
        if (this.body == null) this.body = new ModelRendererBends(argModel);
        this.body.sync((ModelRendererBends) argModel.bipedBody);
        if (this.rightArm == null) this.rightArm = new ModelRendererBends(argModel);
        this.rightArm.sync((ModelRendererBends) argModel.bipedRightArm);
        if (this.leftArm == null) this.leftArm = new ModelRendererBends(argModel);
        this.leftArm.sync((ModelRendererBends) argModel.bipedLeftArm);
        if (this.rightLeg == null) this.rightLeg = new ModelRendererBends(argModel);
        this.rightLeg.sync((ModelRendererBends) argModel.bipedRightLeg);
        if (this.leftLeg == null) this.leftLeg = new ModelRendererBends(argModel);
        this.leftLeg.sync((ModelRendererBends) argModel.bipedLeftLeg);
        if (this.rightForeArm == null) this.rightForeArm = new ModelRendererBends(argModel);
        this.rightForeArm.sync(argModel.bipedRightForeArm);
        if (this.leftForeArm == null) this.leftForeArm = new ModelRendererBends(argModel);
        this.leftForeArm.sync(argModel.bipedLeftForeArm);
        if (this.rightForeLeg == null) this.rightForeLeg = new ModelRendererBends(argModel);
        this.rightForeLeg.sync(argModel.bipedRightForeLeg);
        if (this.leftForeLeg == null) this.leftForeLeg = new ModelRendererBends(argModel);
    ...
    public static EntityData get(int argEntityID) {
                ...
                if (this.currentAttack == 0) {
                    ...
                    if (this.ticksAfterPunch < 15.0f) {
                        if (this.currentAttack == 1) this.currentAttack = 2;
                            ...
                            this.currentAttack = (!ModuleManager.getModule(MoBends.class).spinAttack.isEnabled() || this.getEntity().isRiding()) ? 1 : 3;
